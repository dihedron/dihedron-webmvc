...
//	
	 ...
	 * 
	 * @return
	 *   whether the session id was presented by the client as a parameter in the
	 *   request URL.
	 */
...
//	}
	 ...
	 * to provide a consistent high-level view of parameter and attribute passing.
	 * 
	 * @param key
	 *   the name of the value.
	 * @param scope
	...
	/**
	 * Sets the value associated with the given name into the given scope; despite
	 * making no difference between parameters and attributes from a theoretical
	 * standpoint, this method will actually perform a check to see if an attempt
	 * is being made to store a value in a read-only context, so make sure you do
	 * not try to set a value in FORM or CONFIGURATION scopes as this would result 
	...
	/**
	 * Adds all the entries in the given map into the given scope, overriding any
	...
	/**
	 * Removes the value associated with the given name from the given scope; despite
	 * making no difference between parameters and attributes from a theoretical
	 * standpoint, this method will actually perform a check to see if an attempt
	 * is being made to remove a value from a read-only context, so make sure you do
	 * not try to remove a value from FORM or CONFIGURATION scopes as this would 
	...
	/**
	 * Removes all values whose name matches the given regular expression from the
	 * given scope.
	 * 
	 * @param pattern
	 ...
	 * filtering out those that do not match the given pattern (if provided).
	 * 
	 * @param scope
	 *   the scope whose value (attribute/parameter) names should be retrieved.
	 * @param pattern
	...
	public static Map<String, Object> findValues(Regex pattern, Scope... scopes) throws ZephyrException {
		...
		if(scopes != null && scopes.length > 0) {
			// visit the scopes in reverse order so that first scopes have higher
