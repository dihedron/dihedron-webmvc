...
public class CachingRendererRegistry implements RendererRegistry {
    ...
    @Override
    public void addRenderer(String id, Class<? extends Renderer> clazz) throws WebMVCException {
        if (Strings.isValid(id) && clazz != null) {
            try {
                ...
                throw new WebMVCException("Error instantiating renderer class '" + clazz.getCanonicalName() + "'", e);
            } catch (IllegalAccessException e) {
                logger.error("error accessing class '{}'", clazz.getCanonicalName());
                throw new WebMVCException("Error accessing renderer class '" + clazz.getCanonicalName() + "'", e);
            }
        }
    }
